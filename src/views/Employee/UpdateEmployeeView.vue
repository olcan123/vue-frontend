<template>
  <div class="flex justify-between items-center border-b pb-4 mb-6">
    <h2 class="text-2xl font-bold text-gray-800">Calisan Duzenle</h2>
    <button
...
const { employementTypes, employee } = storeToRefs(employeeStore);
...
setValues(employee.value);

const onSubmit = handleSubmit(async (values) => {
  delete values.id;
  await employeeStore.updateEmployee(id, values);
});
</script>
